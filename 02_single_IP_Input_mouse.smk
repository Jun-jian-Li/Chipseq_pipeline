...
print("单端测序有Input对照样本的命名格式为:Sample_Input.fq.gz")
print("单端测序有Input对照样本的命名格式为:Sample_IP.fq.gz")
...
rule all:
     input:
          #qc
          raw_data_fastqc = expand(os.path.join( config["dir"]["raw_data_qc_result_dir"],"{sample}_{group}_fastqc.html" ),sample=samples,group=groups),
          clean_data = expand(os.path.join( config["dir"]["clean_data_dir"], "{sample}_{group}_trimmed.fq.gz"),sample=samples,group=groups),
          clean_data_fastqc=expand(os.path.join( config["dir"]["clean_data_qc_result_dir"],"{sample}_{group}_trimmed_fastqc.html"),sample=samples,group=groups),
          cut_base_data = expand(os.path.join(config["dir"]["cut_base_dir"],"{sample}_{group}_cut.fq.gz"),sample=samples,group=groups),
          cut_data_fastqc = expand(os.path.join(config["dir"]["cut_data_qc_result_dir"],"{sample}_{group}_cut_fastqc.html"),sample=samples,group=groups),
...
rule raw_fastqc:
     input:
          raw_data = os.path.join(config["dir"]["raw_data_dir"],"{sample}_{group}.fq.gz")
     output:
          raw_data_fastqc = os.path.join(config["dir"]["raw_data_qc_result_dir"],"{sample}_{group}_fastqc.html")
     params:
...
rule trim:
     input:
          raw_data = os.path.join(config["dir"]["raw_data_dir"],"{sample}_{group}.fq.gz")
     output:
          clean_data = os.path.join(config["dir"]["clean_data_dir"],"{sample}_{group}_trimmed.fq.gz")
     message: """-------------------------- Trim. ------------------------------------"""
     params:
...
rule trim_fastqc:
     input:
          clean_data = os.path.join(config["dir"]["clean_data_dir"], "{sample}_{group}_trimmed.fq.gz")
     output:
          clean_data_fastqc=os.path.join(config["dir"]["clean_data_qc_result_dir"],"{sample}_{group}_trimmed_fastqc.html")         
     params:
...
rule cut_base:
     input:
          clean_data = os.path.join(config["dir"]["clean_data_dir"],"{sample}_{group}_trimmed.fq.gz")
     output:
          cut_base_data = os.path.join(config["dir"]["cut_base_dir"],"{sample}_{group}_cut.fq.gz")
     message: """----------------------- Cut base ---------------------------------------."""
     params:
...
rule cut_fastqc:
     input:
          cut_base_data = os.path.join(config["dir"]["cut_base_dir"],"{sample}_{group}_cut.fq.gz")
     output:
          cut_data_fastqc=os.path.join(config["dir"]["cut_data_qc_result_dir"],"{sample}_{group}_cut_fastqc.html")
     params:
